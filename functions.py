...
def newton_solver_FOM(mu0, mu1, geometry_utilities, vtk_utilities, mesh, mesh_geometric_data, tot_dofs, export_solution_path,
                       speed_n_dofs, speed_mesh_dofs_info, speed_dofs_data, speed_reference_element_data, speed_n_strongs,
                       pressure_n_dofs, pressure_mesh_dofs_info, pressure_dofs_data, pressure_reference_element_data, pressure_n_strongs,plot=False):
    ...
    if plot:
        plot_solution(mu0, mu1, u_k, "u_x FOM", "u_y FOM", "u_mag FOM", "p FOM",
                    geometry_utilities, vtk_utilities, mesh, mesh_geometric_data, tot_dofs, export_solution_path,
                    speed_n_dofs, speed_mesh_dofs_info, speed_dofs_data, speed_reference_element_data, speed_n_strongs,
                    pressure_n_dofs, pressure_mesh_dofs_info, pressure_dofs_data, pressure_reference_element_data, pressure_n_strongs)
    
    return u_k,X_1,X_2,B_1,B_2

def newton_solver_ROM(mu0,mu1,newton_tol,max_iterations,basis,
                      geometry_utilities, vtk_utilities, mesh, mesh_geometric_data, tot_dofs, export_solution_path,
                      speed_n_dofs, speed_mesh_dofs_info, speed_dofs_data, speed_reference_element_data, speed_n_strongs,
                      pressure_n_dofs, pressure_mesh_dofs_info, pressure_dofs_data, pressure_reference_element_data, pressure_n_strongs, plot=False):
    ...
    ROM_tot_dofs = basis.shape[1]
    u_N_k = np.zeros(ROM_tot_dofs)
    
    u_k = basis @ u_N_k # Proiezione nello spazio completo per calcolare i residui
    u_x_numeric = u_k[0:speed_n_dofs]
    u_y_numeric = u_k[speed_n_dofs:2 * speed_n_dofs]
    p_numeric = u_k[2 * speed_n_dofs:]
    ...
    while num_iteration < max_iterations and residual_norm > newton_tol * solution_norm:
        ...
        # Risoluzione del sistema lineare ridotto e aggiornamento
        J_delta_ROM = basis.transpose() @ (J_delta @ basis)
        G_delta_ROM = basis.transpose() @ J_f 
        du_N = np.linalg.solve(J_delta_ROM, G_delta_ROM)
        u_N_k = u_N_k + du_N

        # Riproiezione di V_delta
        du = basis @ du_N
        u_k = basis @ u_N_k
        ...
        # Calcolo gli errori usando i vettori FOM
        # devo far ritornare da speed_x_exact, speed_y_exact e speed_z_exact la soluzione FOM!!!
        du_x_error_L2 = polydim.pde_tools.assembler_utilities.pcc_2_d.compute_error_l2(geometry_utilities,
                                                                                        mesh,
                                                                                        mesh_geometric_data,
                                                                                        speed_dofs_data,
        ...
        solution_norm = np.sqrt(u_x_error_L2.numeric_norm_l2**2 + 
                                u_y_error_L2.numeric_norm_l2**2 +
                                p_error_L2.numeric_norm_l2**2)
                                
        residual_norm = np.sqrt(du_x_error_L2.numeric_norm_l2**2 + 
                                du_y_error_L2.numeric_norm_l2**2 +
                                dp_error_L2.numeric_norm_l2**2)

        print("dofs", "u_x_errorL2", "u_y_errorL2", "p_errorL2", "residual", "iteration", "max_iteration")
        print(ROM_tot_dofs, '{:.2e}'.format(u_x_error_L2.error_l2 / u_x_error_L2.numeric_norm_l2), 
              '{:.2e}'.format(u_y_error_L2.error_l2 / u_y_error_L2.numeric_norm_l2), 
              '{:.2e}'.format(p_error_L2.error_l2 / p_error_L2.numeric_norm_l2), 
              '{:.16e}'.format(residual_norm / solution_norm), '{:d}'.format(num_iteration), '{:d}'.format(max_iterations))
        
        num_iteration = num_iteration + 1

    if plot:
        plot_solution(mu0, mu1, u_k, "u_x ROM", "u_y ROM", "u_mag ROM", "p ROM",
                    geometry_utilities, vtk_utilities, mesh, mesh_geometric_data, tot_dofs, export_solution_path,
                    speed_n_dofs, speed_mesh_dofs_info, speed_dofs_data, speed_reference_element_data, speed_n_strongs,
                    pressure_n_dofs, pressure_mesh_dofs_info, pressure_dofs_data, pressure_reference_element_data, pressure_n_strongs)
...
def generate_snapshots(snapshot_num, geometry_utilities, vtk_utilities, mesh, mesh_geometric_data, tot_dofs, export_solution_path,
    ...
    for mu_arr in tqdm(training_set, desc="Generating Snapshots"):
        ...
        u_k,X_1,X_2,B_1,B_2 = newton_solver_FOM(mu0, mu1, geometry_utilities, vtk_utilities, mesh, mesh_geometric_data, tot_dofs, export_solution_path,
                                                speed_n_dofs, speed_mesh_dofs_info, speed_dofs_data, speed_reference_element_data, speed_n_strongs,
                                                pressure_n_dofs, pressure_mesh_dofs_info, pressure_dofs_data, pressure_reference_element_data, pressure_n_strongs)
...
def plot_solution(mu0, mu1, u_k, title_ux, title_uy, title_umag, title_p,
                    geometry_utilities, vtk_utilities, mesh, mesh_geometric_data, tot_dofs, export_solution_path,
                    speed_n_dofs, speed_mesh_dofs_info, speed_dofs_data, speed_reference_element_data, speed_n_strongs,
                    pressure_n_dofs, pressure_mesh_dofs_info, pressure_dofs_data, pressure_reference_element_data, pressure_n_strongs):
    
    reduced_nsup_u_x = u_k[0:speed_n_dofs]
    reduced_nsup_u_y = u_k[speed_n_dofs:2 * speed_n_dofs]
    reduced_nsup_p = u_k[2 * speed_n_dofs:]
    ...
    reduced_nsup_u_x_on_cell0Ds = polydim.pde_tools.assembler_utilities.pcc_2_d.extract_solution_on_cell0_ds(mesh,
                                                                        speed_dofs_data,
                                                                        reduced_nsup_u_x,
                                                                        u_x_strong)
    reduced_nsup_u_y_on_cell0Ds = polydim.pde_tools.assembler_utilities.pcc_2_d.extract_solution_on_cell0_ds(mesh,
                                                                        speed_dofs_data,
                                                                        reduced_nsup_u_y,
                                                                        u_y_strong)
    reduced_nsup_p_on_cell0Ds = polydim.pde_tools.assembler_utilities.pcc_2_d.extract_solution_on_cell0_ds(mesh,
                                                                        pressure_dofs_data,
                                                                        reduced_nsup_p,
                                                                        p_strong)
    vtk_utilities.export_solution_2(export_solution_path + '/reduced_nsup_u_x',
                                mesh, 
                                reduced_nsup_u_x_on_cell0Ds.numeric_solution)
    vtk_utilities.export_solution_2(export_solution_path + '/reduced_nsup_u_y',
                                mesh, 
                                reduced_nsup_u_y_on_cell0Ds.numeric_solution)
    vtk_utilities.export_solution_2(export_solution_path + '/reduced_nsup_p',
                                mesh, 
                                reduced_nsup_p_on_cell0Ds.numeric_solution)
    other_ut.plot_solution(mesh, reduced_nsup_u_x_on_cell0Ds.numeric_solution, title_ux) 
    other_ut.plot_solution(mesh, reduced_nsup_u_y_on_cell0Ds.numeric_solution, title_uy)
    other_ut.plot_solution(mesh, np.sqrt(reduced_nsup_u_x_on_cell0Ds.numeric_solution * reduced_nsup_u_x_on_cell0Ds.numeric_solution + reduced_nsup_u_y_on_cell0Ds.numeric_solution * reduced_nsup_u_y_on_cell0Ds.numeric_solution), title_umag)
    other_ut.plot_solution(mesh, reduced_nsup_p_on_cell0Ds.numeric_solution, title_p) 